-- Tables added after the baseline Template.sql.  The MariaDB image executes
-- this file only when the database volume is empty.

CREATE TABLE IF NOT EXISTS `SystemSetting` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `SettingKey` VARCHAR(100) NOT NULL,
  `SettingValue` TEXT NULL DEFAULT '',
  `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `SystemSetting_SettingKey_unique` (`SettingKey`)
