<template>
  <div>
    ...
    <div style="overflow-y: scroll !important">
      <v-row dense>
        <v-col cols="12" md="6" v-for="experimenter in Experimenters" :key="experimenter.id" dense>
          <v-card class="child-card d-flex flex-column">
            ...
            <v-card-text class="body-1" align="start" style="padding: 8px; color: var(--v-primary)">{{ "Email: " +
              experimenter.Email }}</v-card-text>
      ...
      <div>
        <v-dialog v-model="dialogExperimenter" max-width="800px">
          <v-card>
            ...
            <v-card-text>
              <v-row align="center" justify="center" style="height: 200px;" dense>
                <v-col cols="12" lg="10">
                  <v-select :items="labMembers" :item-value="'id'" :item-text="'Name'" v-model="editedExperimenter"
                    return-object label="Experimenters" multiple chip outlined dense></v-select>
                </v-col>
              </v-row>
            </v-card-text>
