...
exports.studyStats = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    var queryStringNWeeklyRecrtuiment = "SELECT     YEAR(${{DBName}}.Schedule.updatedAt) AS Year,     WEEK(${{DBName}}.Schedule.updatedAt) AS weekIndex,     ${{DBName}}.Schedule.Status AS Status,     COUNT(DISTINCT ${{DBName}}.Appointment.id) AS NumberOfParticipants,     DATE_FORMAT( DATE_SUB( ${{DBName}}.Schedule.updatedAt,  interval WEEKDAY(${{DBName}}.Schedule.updatedAt) DAY  ), '%Y-%m-%d' ) AS WeekStartDate FROM     ${{DBName}}.Appointment     INNER JOIN ${{DBName}}.Schedule ON ${{DBName}}.Appointment.FK_Schedule = ${{DBName}}.Schedule.id     INNER JOIN ${{DBName}}.Study ON ${{DBName}}.Appointment.FK_Study = ${{DBName}}.Study.id     INNER JOIN ${{DBName}}.Lab ON ${{DBName}}.Study.FK_Lab = ${{DBName}}.Lab.id     INNER JOIN ${{DBName}}.Family ON ${{DBName}}.Schedule.FK_Family = ${{DBName}}.Family.id WHERE     ${{DBName}}.Schedule.createdAt > '2021-01-01'     AND ${{DBName}}.Study.id = ${{studyID}}     AND ${{DBName}}.Family.TrainingSet = 0 GROUP BY     Year,     Status,     weekIndex, WeekStartDate ORDER BY     Year,     weekIndex;";
