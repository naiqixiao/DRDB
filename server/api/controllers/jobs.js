...
exports.updateScheduledJob = async (req, res) => {
  ...
  try {
    const updatedJob = await updateScheduledJob(jobId, { cron, enabled }, personnel.FK_Lab);
    return res.status(200).json(updatedJob);
  } catch (error) {
    if (error.message === "Job not found") {
      return res.status(404).json({ message: error.message });
    }
    if (error.message === "Job is not editable" || error.message === "Invalid cron expression" || error.message === "Lab ID is required") {
